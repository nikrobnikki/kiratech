...
const { User, ServiceRequest, Service, Technician, Notification, Review, Payment, ChatMessage, TechnicianMessage } = require('../models');
 ...
 *   delete:
 *     summary: Delete a completed or cancelled service request
 *     tags: [User]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *                 message: { type: string }
 *       400:
 *         description: Only completed or cancelled requests can be deleted
 *       404:
 *         description: Request not found
 */
router.delete('/requests/:id', async (req, res) => {
  try {
    const request = await ServiceRequest.findOne({ where: { id: req.params.id, userId: req.user.id } });
    if (!request) return res.status(404).json({ error: 'Request not found' });
    if (!['completed', 'cancelled'].includes(request.status)) {
      return res.status(400).json({ error: 'Only completed or cancelled requests can be deleted' });
    }

    // Delete all related records first (cascade-safe order)
    await Promise.all([
      Review.destroy({ where: { requestId: request.id } }),
      Payment.destroy({ where: { requestId: request.id } }),
      ChatMessage.destroy({ where: { requestId: request.id } }),
      TechnicianMessage.destroy({ where: { requestId: request.id } }),
      Notification.destroy({ where: { relatedId: request.id } }),
    ]);
