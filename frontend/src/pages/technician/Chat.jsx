  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col h-[calc(100vh-6rem)]">
      <div className="flex items-center gap-4 mb-4">
        <Link to={`/technician/tasks/${id}`} className="text-slate-400 hover:text-white">←</Link>
        <div>
          <h1 className="font-bold text-white">Chat — {meta?.ticketNumber}</h1>
          <p className="text-xs text-slate-400">{meta?.customer ? `Customer: ${meta.customer.name}` : ''}</p>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto space-y-3 bg-slate-900 rounded-xl border border-slate-800 p-4">
        {messages.length === 0 && <p className="text-center text-slate-500 py-10">No messages yet.</p>}
        {messages.map(m => {
          const isMe = m.senderId === myId;
          return (
            <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${isMe ? 'bg-purple-600 text-white rounded-br-sm' : 'bg-slate-800 text-slate-100 rounded-bl-sm'}`}>
      ...
      <form onSubmit={send} className="flex gap-3 mt-4">
        <input value={text} onChange={e => setText(e.target.value)} className="input-field flex-1"
