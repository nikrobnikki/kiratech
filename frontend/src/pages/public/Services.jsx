...
// ─── Single service card ───────────────────────────────────────────────────────
function ServiceCard({ svc, index, highlighted }) {
  const [expanded, setExpanded] = useState(false);
  const [hovered, setHovered] = useState(false);
  const subTypes = serviceSubTypes[svc.name];
  const showSubTypes = subTypes && (expanded || hovered);
  ...
  return (
    <div
      ...
      className={`card-cyber p-5 group transition-all duration-300 animate-fade-in-up
                  ...
                  ${highlighted ? 'border-blue-500/40 border-2 shadow-cyber-lighter' : ''}
...
export default function Services() {
  ...
  useEffect(() => {
    ...
    const target = services.find(s => s.name === focus);
    setFocusedService(target || null);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {filtered.map((svc, i) => (
                <ServiceCard
                  ...
                  svc={svc}
                  index={i}
                  highlighted={focusedService?.name === svc.name}
                />
              ))}
            </div>
          )}
